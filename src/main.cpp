...
int pIndex = 0;
int address = 0; //Used to keep a pointer for dumping eeprom
unsigned int currentMemoryAddress = 0; // Track what address we last saved data to
...
void setup() {
  ...
  pReadCharacteristic->setValue(dataSize);
...
int bmp180Temp(){
  ...
  // cast as int because I only need the whole number temperature
  int temp = bmp180.getTemperature();
  return c2f(temp);
}

/**
 * Save the temperature data to disk and increment the address pointer
 */
void saveTemperatureData(int temperature){
  writeEEPROM(eeprom, currentMemoryAddress++, temperature);

  // Let BLE know the current size
  dataSize = currentMemoryAddress;
  pReadCharacteristic->setValue(dataSize);
  Serial.print("Saved data, new size is: ");
  Serial.println(currentMemoryAddress);
}
...
void loop() {
  ...
  // Save current temp every 2000 ms
  if(currentMillis > time_now + 2000) {
    int temperature = bmp180Temp();

    if (temperature > 0){
      // Save it to eeprom
      saveTemperatureData(temperature);

      /* If a BLE client is connected and we're not dumping data send the current temperature to it */
      if (deviceConnected && !dataDump) {
        Serial.print("Device is connected, sending current temp: ");
        Serial.println(temperature);

        uint8_t buffer[1] = { (uint8_t)temperature };
        // buffer[0] = temperature;
        pNotifyCharacteristic->setValue(buffer, 1);
